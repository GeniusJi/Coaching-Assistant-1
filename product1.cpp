...
int main() {
	...
	for (int i = 0; i < playerCount; i++) trainData[i] = players[i];
	trainCount = playerCount;
	...
	for (int i = 0; i < FEATURES; i++) {
		double total;
		cout << featureNames[i] << " (season total): ";
		if (!(cin >> total)) {
			cout << "Invalid input.\n";
			return 0;
		}
		inputPlayer.stat[i] = total * 90.0 / minutes;
	}

	int votes[3] = {};
	for (int tree = 0; tree < TREES; tree++) {
		int prediction = predict(forest[tree], inputPlayer);
		votes[prediction]++;
	}
	...
	for (int position = 0; position < 3; position++) {
		cout << getPositionName(position) << ": "
		     << (100.0 * votes[position] / TREES) << "%\n";
	}

	int best = 0;
	for (int position = 1; position < 3; position++) {
		if (votes[position] > votes[best]) {
			best = position;
		}
	}

	cout << "Most similar position: " << getPositionName(best) << '\n';
}
